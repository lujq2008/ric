...
typedef struct RICcontrolAckMessage {
	long requestorID;
	long requestSequenceNumber;
	long ranfunctionID;
	RICcallProcessID_t	 RICcallProcessID;
	long	 RICcontrolStatus;
...
/* RICcontrolRequest */
long e2ap_get_ric_control_request_sequence_number(void* buffer, size_t buf_size);
ssize_t  e2ap_set_ric_control_request_sequence_number(void* buffer, size_t buf_size, long sequence_number);
ssize_t e2ap_encode_ric_control_request_message(void* buffer, size_t buf_size, long ricRequestorID, long ricRequestSequenceNumber, long ranFunctionID, 
	void* ricControlHeader,long ricControlHeaderLen,void* ricControlMessage, long ricControlMessageLen);

/* RICcontrolResponse */
long e2ap_get_ric_control_response_sequence_number(void* buffer, size_t buf_size);
ssize_t  e2ap_set_ric_control_response_sequence_number(void* buffer, size_t buf_size, long sequence_number);
RICcontrolAckMsg* e2ap_decode_ric_control_response_message(void* buffer, size_t buf_size);

/* RICcontrolFailure */
long e2ap_get_ric_control_failure_sequence_number(void* buffer, size_t buf_size);
