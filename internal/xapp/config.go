	...
	updateMTypes := func() {
		...
		if len(mtypes) > 0 {
			...
			for _, v := range mtypes {
				...
				if iadd != nadd {
					l.Error("Config mtypes mtypes entry skipped due conflict with existing values %s(%t) %d(%t) ", v.Name, nadd, v.Id, iadd)
				} else if iadd {
					l.Info("Config mtypes mtypes entry added %s(%t) %d(%t) ", v.Name, nadd, v.Id, iadd)
					RICMessageTypes[v.Name] = int(v.Id)
					RicMessageTypeToName[int(v.Id)] = v.Name
				} else {
					l.Info("Config mtypes mtypes entry skipped %s(%t) %d(%t) ", v.Name, nadd, v.Id, iadd)
				}
			}
...
func (*Configurator) SetSDLNotificationCB(appName string, sdlNotificationCb SDLNotificationCB) error {
	return nil //Sdl.Subscribe(sdlNotificationCb, fmt.Sprintf("CM_UPDATE:%s", appName))
}
