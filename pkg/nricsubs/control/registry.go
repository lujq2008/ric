...
func (r *Registry) AssignToSubscription(trans *TransactionXapp, subReqMsg *e2ap.E2APSubscriptionRequest) (*Subscription, error) {
	...
	//
	RouteTable := &pbdb.RouteTable{SubIdXapp: int64(trans.SubId),Topic: trans.XappKey.Endpoint.String(),SubIdRan: int64(subs.ReqId.InstanceId)}
	if epamount == 1 {
		req := &pbdb.RouteTableInsertRequest{RouteTable:RouteTable}
		_,err = r.AcessDbAgent.Client.RouteTableInsert(context.Background(),req)
	} else {
		req := &pbdb.RouteTableUpdateRequest{RouteTable:RouteTable}
		_,err = r.AcessDbAgent.Client.RouteTableUpdate(context.Background(),req)
	}
	r.mutex.Lock()
